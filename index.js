...
const publicUrl =
  env === "development" ? "http://localhost:3000" : process.env.PUBLIC_URL;
...
app.post("/myprofile", authorization, async (req, res) => {
  try {
    const participant = await pool.query(
      "SELECT * FROM salesforce.participant__c WHERE ExternalId__c = $1",
      [req.user.id]
    );
...
app.get("/participants", async (req, res) => {
  try {
    const allParticipants = await pool.query(
      "SELECT * FROM salesforce.participant__c"
    );
...
app.post("/participations", authorization, async (req, res) => {
  try {
    ...
    const part = await pool.query(
      "SELECT * FROM salesforce.participation__c WHERE contest__c = $1 AND participant__r__externalid__c = $2",
...
app.post("/participationswronganswer", async (req, res) => {
  try {
    ...
    const participationWrongAnswer = await pool.query(
      "SELECT * FROM salesforce.participation__c WHERE externalid__c = $1",
      [partid]
    );

    console.log(
      "parts wrong answers" + JSON.stringify(participationWrongAnswer.rows)
    );
...
app.get("/mycontests", authorization, async (req, res) => {
  try {
    ...
    const mycontests = await pool.query(
      "SELECT * FROM salesforce.participation__c AS participation, salesforce.contest__c AS contest WHERE participation.participant__r__externalid__c = $1 AND contest.sfid = participation.contest__c",
      [req.user.id]
    );
...
app.get("/allcontests", authorization, async (req, res) => {
  try {
    ...
    const allContests = await pool.query(
      "SELECT * FROM salesforce.contest__c WHERE status__c != 'Finished' AND start_time__c > now()"
    );
...
app.get("/event/:id", authorization, async (req, res) => {
  try {
    ...
    const event = await pool.query(
      "SELECT * FROM salesforce.event__c AS event, salesforce.team__c AS team WHERE event.sfid = $1 AND (event.home_team__c = team.sfid OR event.away_team__c = team.sfid)",
      [id]
    );
    res.json(event.rows);
  } catch (err) {
...
app.get("/contestdetail/:id", async (req, res) => {
  try {
    ...
    const contest = await pool.query(
      "SELECT * FROM salesforce.contest__c WHERE sfid = $1",
      [id]
    );
...
app.get(
  "/contestparticipations/:contest_id",
  authorization,
  async (req, res) => {
    try {
      ...
      const part = await pool.query(
        "SELECT * FROM salesforce.participant__c AS participant, salesforce.participation__c AS participation WHERE participation.contest__c = $1 AND participation.participant__r__externalid__c = participant.externalid__c::text;",
        [contest_id]
      );
      res.json(part.rows);
    } catch (err) {
...
app.get(
  "/participationbycontest/:contest_id",
  authorization,
  async (req, res) => {
    try {
      const { contest_id } = req.params;
...
app.post("/disablequestions/", authorization, async (req, res) => {
  try {
    ...
    const allContestQuestions = await pool.query(
      "UPDATE salesforce.question__c SET islocked__c = true WHERE published__c = true AND contest__c = $1 RETURNING *",
      [conid]
    );
    ...
    const selectQuestions = await pool.query(
      "SELECT * FROM salesforce.question__c WHERE sfid = ANY ($1) ORDER BY Name ASC",
      [idlist]
    );
...
app.post("/countsubsegment/", authorization, async (req, res) => {
  try {
    ...
    const subsegQuestions = await pool.query(
      "SELECT * FROM salesforce.question__c WHERE contest__c = $1 AND published__c = true AND SubSegment__c = $2",
      [conid, subseg]
    );
...
app.get("/allquestions/:contest_id", authorization, async (req, res) => {
  try {
    ...
    const allContestQuestions = await pool.query(
      "SELECT * FROM salesforce.question__c WHERE contest__c = $1 ORDER BY Name ASC",
      [contest_id]
    );
...
app.get(
  "/allendingparticipations/:contest_id",
  authorization,
  async (req, res) => {
    try {
      const { contest_id } = req.params;
...
app.post("/markcorrect", authorization, async (req, res) => {
  try {
    ...
    const selectedpartanswers = await pool.query(
      "SELECT * FROM salesforce.participation_answers__c WHERE question__c = $1",
      [questsfid]
    );
    var incorrectlist = [];
    var partidlist = [];
    ...
    for (var i = 0; i < selectedpartanswers.rows.length; i++) {
      ...
      const selectedpartanswers2 = await pool.query(
        "UPDATE salesforce.participation_answers__c SET validated__c = $1, incorrect__c = $2, status__c = $3, correct__c = $4 WHERE id = $5",
        [
          selectedpartanswers.rows[i].validated__c,
          selectedpartanswers.rows[i].incorrect__c,
          selectedpartanswers.rows[i].status__c,
          selectedpartanswers.rows[i].correct__c,
          selectedpartanswers.rows[i].id,
        ]
      );
      console.log(selectedpartanswers2.rows);
    }
    console.log("check 2");
    const selectedpartanswers1 = await pool.query(
      "SELECT * FROM salesforce.participation_answers__c WHERE question__c = $1",
      [questsfid]
    );
    ...
    const incorrectparts = await pool.query(
      "SELECT * FROM salesforce.participation__c WHERE sfid = ANY ($1)",
      [partidlist]
    );
    ...
    for (var i = 0; i < incorrectparts.rows.length; i++) {
      for (var k = 0; k < incorrectlist.length; k++) {
        ...
        if (incorrectparts.rows[i].sfid == incorrectlist[k].participation__c) {
          ...
          console.log(incorrectparts.rows[i].wrong_answers__c);
          if (
            incorrectparts.rows[i].wrong_answers__c ==
            con.wrong_answers_allowed__c
          ) {
            incorrectparts.rows[i].status__c = "Knocked Out";
          }
        }
        const incorrectrows = await pool.query(
          "UPDATE salesforce.participation__c SET wrong_answers__c = $1 WHERE id = $2 RETURNING *",
          [incorrectparts.rows[i].wrong_answers__c, incorrectparts.rows[i].id]
        );
    ...
    const allcontestquestions = await pool.query(
      "SELECT * FROM salesforce.question__c WHERE contest__c = $1 AND correct_answer__c IS NOT NULL",
      [con.sfid]
    );
    const activeparts = await pool.query(
      "SELECT * FROM salesforce.participation__c WHERE status__c = 'Active' AND contest__c = $1",
      [con.sfid]
    );
    ...
    if (
      con.number_of_questions__c == allcontestquestions.rows.length ||
      activeparts.rows.length == 1
    ) {
      console.log("check 4");
      ...
      const finishedparts = await pool.query(
        "SELECT * FROM salesforce.participation__c WHERE contest__c = $1 ORDER BY Wrong_Answers__c ASC",
        [con.sfid]
      );
      var place = 1;
      ...
      for (var i = 0; i < finishedparts.rows.length; i++) {
        if (index > 0 && indexless >= 0) {
          if (
            finishedparts.rows[index].wrong_answers__c >
            finishedparts.rows[indexless].wrong_answers__c
          ) {
        ...
        const allfinishedparts = await pool.query(
          "UPDATE salesforce.participation__c SET placefinish__c = $1 WHERE id = $2 RETURNING *",
          [finishedparts.rows[i].placefinish__c, finishedparts.rows[i].id]
        );
        if (finishedparts.rows[i].placefinish__c == 1) {
          participantid = finishedparts.rows[i].participant__c;
          console.log("part:::" + participantid);
          var winval;
          const contestswon = await pool.query(
...
app.get("/questions/:contest_id", authorization, async (req, res) => {
  try {
    ...
    const allContestQuestions = await pool.query(
      "SELECT * FROM salesforce.question__c WHERE contest__c = $1 AND published__c = true ORDER BY Name ASC",
      [contest_id]
    );
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const participationExistAnswer = await pool.query(
        "SELECT * FROM salesforce.participation_answers__c WHERE participation__c = $1 AND question__c = $2 ",
        [partsfid, questid]
      );
...
app.post("/existingpartanswernoquestion/", authorization, async (req, res) => {
  try {
    ...
    const participationAnswer = await pool.query(
      "SELECT * FROM salesforce.participation_answers__c WHERE participation__c = $1 ORDER BY name ASC",
      [partsfid]
    );
  ...
  try {
    ...
    for (var i = 0; i < partanswers.length; i++) {
      ...
      const partans = await pool.query(
        "UPDATE salesforce.Participation_Answers__c SET selection__c = $1, selection_value__c = $2, Status__c = $3, ExternalId__c = gen_random_uuid() WHERE Participation__c = $4 AND Question__c = $5 RETURNING *",
        [
          answer.selection__c,
          answer.selection_value__c,
          "Submitted",
          answer.participation__c,
          answer.question__c,
        ]
      );
    ...
    const part = await pool.query(
      "UPDATE salesforce.Participation__c SET Questions_Submitted__c = true WHERE sfid = $1",
      [participationrec]
    );
...
app.post("/publishcontest", authorization, async (req, res) => {
  try {
    ...
    const pubquest = await pool.query(
      "UPDATE salesforce.Question__c SET published__c = true WHERE contest__c = $1 RETURNING *",
      [contest_id]
    );
