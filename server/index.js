...
app.get("/mycontests", authorization, async (req, res) => {
  try {
    ...
    const mycontests = await pool.query(
      "SELECT * FROM salesforce.participation__c AS participation, salesforce.contest__c AS contest WHERE external_participant__c = $1 AND contest.sfid = participation.contest__c",
      [req.user.id]
    );
...
app.get("/contest/:id/", async (req, res) => {
  try {
    ...
    const contest = await pool.query(
      "SELECT * FROM salesforce.contest__c WHERE id = $1",
      [id]
    );
...
app.post("/participations", authorization, async (req, res) => {
  try {
    ...
    const part = await pool.query(
      "SELECT * FROM salesforce.participation__c WHERE contest__c = $1 AND external_participant__c = $2",
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const part = await pool.query(
        "SELECT * FROM salesforce.participation__c AS participation, salesforce.participant__c AS participant WHERE participation.contest__c = $1 AND participation.external_participant__c = participant.participant_id::text;",
        [contest_id]
      );
      res.json(part.rows);
...
app.get(
  ...
  async (req, res) => {
    try {
      const { contest_id } = req.params;
      const part = await pool.query(
        "SELECT * FROM salesforce.participation__c WHERE contest__c = $1 AND ExternalId__c = $2",
        [contest_id, req.user.id]
      );
      res.json(part.rows[0]);
    } catch (err) {
...
app.get("/participations/:id", async (req, res) => {
  try {
    ...
    const part = await pool.query(
      "SELECT * FROM salesforce.participation__c WHERE id = $1",
      [id]
    );
    res.json(part.rows[0]);
  } catch (err) {
...
app.get("/questions/:contest_id", authorization, async (req, res) => {
  try {
    ...
    const allContestQuestions = await pool.query(
      "SELECT * FROM salesforce.question__c WHERE contest__c = $1 AND published__c = true",
      [contest_id]
    );
...
app.post("/disablequestions/", async (req, res) => {
  try {
    ...
    const allContestQuestions = await pool.query(
      "UPDATE salesforce.question__c SET IsLocked__c = true WHERE sfid = ANY ($1)",
      [questionids]
    );
    res.json(allContestQuestions.rows);
